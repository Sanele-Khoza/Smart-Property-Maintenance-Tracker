...
2. **Provider "Decline job" implemented** — spec UC-004 alt flow requires Accept + Decline (+ reason); both now wired: `PUT /tickets/:id/decline` (SP/SA, assigned provider only) moves the ticket to `Declined`, clears `assigned_to`, and records an optional `postponeUntil` + `note`. UI: Decline buttons in `provider/MyJobs.jsx` and `provider/JobDetail.jsx`; manager sees a `Declined` tab + postpone info and reassigns via the existing reassign modal.
