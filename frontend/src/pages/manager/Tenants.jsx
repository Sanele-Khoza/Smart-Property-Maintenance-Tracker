                      ...
                      <tr className="expanded-row"><td colSpan="6" style={{ padding: '8px 16px', backgroundColor: 'var(--surface)' }}>
                        ...
                        {tenant.tickets.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>No tickets.</div> : (
                          ...
                          <table style={{ width: '100%', fontSize: 11, borderCollapse: 'collapse' }}>
                            ...
                            <tbody>{tenant.tickets.map(t => (<tr key={t.ticketId} style={{ borderBottom: '1px solid var(--border)' }}><td className="cell-mono" style={{ padding: '4px 8px' }}>{t.ticketId}</td><td style={{ padding: '4px 8px' }}>{t.title}</td><td style={{ padding: '4px 8px' }}><span className={`badge ${t.status === 'Closed' || t.status === 'Completed' || t.status === 'Tenant Confirmed' ? 'badge-completed' : t.status === 'Assigned' || t.status === 'In Progress' ? 'badge-info' : 'badge-warning'}`}>{t.status}</span></td><td style={{ padding: '4px 8px' }}><span className={`badge ${t.priority === 'URGENT' ? 'badge-danger' : t.priority === 'HIGH' ? 'badge-warning' : t.priority === 'MEDIUM' ? 'badge-info' : 'badge-completed'}`}>{t.priority}</span></td><td className="cell-mono" style={{ padding: '4px 8px' }}>{t.unitNumber}</td></tr>))}</tbody>
                          </table></div>
                        )}
                      </td></tr>
                    )}
