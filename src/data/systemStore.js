...
export const updateSystemSetting = (key, newValue) => {
  const store = getStore();
  const setting = store.systemSettings.find(s => s.key === key);
  if (!setting) return { success: false, error: `Setting "${key}" not found.` };
...
export const updateSlaConfig = (priority, updates) => {
  const store = getStore();
  const sla = store.slaConfig.find(s => s.priority === priority);
  if (!sla) return { success: false, error: `SLA config for "${priority}" not found.` };
